...
def get_dashboard_context(user, portfolio_id=None):
    ...
    context = {
        ...
        # Zabezpieczenie przed dzieleniem przez zero przy day change pct
        'tile_day_pct_str': fmt_2(
            (stats['day_change_pln'] / (stats['total_value'] - stats['day_change_pln']) * 100)
            if (stats['total_value'] - stats['day_change_pln']) > 0.01 else 0
        ),
        'tile_day_pct_raw': (stats['day_change_pln'] / (stats['total_value'] - stats['day_change_pln']) * 100)
        if (stats['total_value'] - stats['day_change_pln']) > 0.01 else 0,
...
# =========================================================
# POZOSTAŁE FUNKCJE (Chart, Details)
# =========================================================
...
def get_asset_details_context(user, symbol, portfolio_id=None):
    ...
    if asset.currency != 'PLN':
        multiplier = rates.get(asset.currency, 1.0)
        if asset.currency == 'JPY': multiplier = rates.get('JPY', 1.0) / 100.0
    ...
    try:
        if not hist_df.empty:
            # FIX: market.py zwraca teraz zawsze MultiIndex [(Ticker, 'Close'), ...]
            # Jeśli ticker się zgadza, bierzemy jego dane
            ...
            if series is not None and not series.empty:
                ...
                # FIX 1: Indeks to obiekty date, a nie Timestamp -> używamy map/list comprehension
                chart_dates = [d.strftime('%Y-%m-%d') for d in series.index]
                # FIX 2: Ceny mnożymy przez walutę
    ...
    # Fallback ceny jeśli 0 (np. błąd pobierania), używamy średniej ceny zakupu
    ...
    for t in asset_data['trades']:
        history_table.append({
            ...
            'price_original': f"{t.get('price', 0):.2f}",
            'currency': 'PLN',  # Tu uproszczenie, bo w bazie amount jest w PLN
    ...
    # Generowanie kolorów kropek na wykresie
    chart_point_colors = []
    chart_point_radius = []

    # Musimy dopasować kropki do dat wykresu
    for d in chart_dates:
        if d in trade_events:
            if trade_events[d] == 'BUY':
                chart_point_colors.append('#00ff7f')
            else:
                chart_point_colors.append('#ff4d4d')
    ...
    return {
        ...
        'day_change_pln': fmt_2(day_change_pln),
        'day_change_pln_raw': day_change_pln,
