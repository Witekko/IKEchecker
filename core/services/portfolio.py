...
def enrich_assets_context(context, assets, total_portfolio_value, watchlist_assets=None):
    ...
    for a in assets:
        ...
        item = {
            ...
            'cur_price_str': fmt_2(a['cur_price']),
            'current_price_fmt': fmt_2(a['cur_price']),
        ...
        if a['is_foreign']:
            if is_etf: foreign_etfs.append(item)
            else: foreign_stocks.append(item)
        else:
            if is_etf: pln_etfs.append(item)
            else: pln_stocks.append(item)

    for lst in [pln_stocks, pln_etfs, foreign_stocks, foreign_etfs]:
        lst.sort(key=lambda x: x['share_pct_raw'], reverse=True)
    ...
    context['pln_stocks'] = pln_stocks
    context['pln_etfs'] = pln_etfs
    context['foreign_stocks'] = foreign_stocks
    context['foreign_etfs'] = foreign_etfs
    context['closed_items'] = closed_items
    
    context['pln_stocks_stats'] = stats(pln_stocks)
    context['pln_etfs_stats'] = stats(pln_etfs)
    ...
    if watchlist_assets:
        for w_asset in watchlist_assets:
            ...
            watchlist_items.append({
                ...
                'current_price_fmt': fmt_2(cur_price),
