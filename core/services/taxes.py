...
def _calculate_ike_tax_shield(transactions, current_value):
    ...
    for t in transactions:
        ...
        if t.type == 'DEPOSIT':
            ...
            # Zaokrąglamy każdy "zaoszczędzony" podatek osobno, tak jak robiłby to system bankowy
    ...
    exit_tax = 0.0
    if total_gain > 0:
        # Podatek liczymy od podstawy, zaokrąglając wynik końcowy
...
def _calculate_standard_tax_report(transactions):
    ...
    for y in sorted(years_db.keys(), reverse=True):
        ...
        # Podatek od akcji (zaokrąglamy do grosza)
        stock_tax = round(max(0.0, d['income'] * 0.19), 2)

        # Podatek od dywidend (zaokrąglamy do grosza)
